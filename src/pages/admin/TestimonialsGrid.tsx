...
const TestimonialsGrid = ({ items, loading, error, onRefresh }: Props) => {
  ...
  const openEdit = (item: GalleryItem) => {
    ...
    setExistingThumbnailUrl(`https://kairafabrics.s3.ap-south-1.amazonaws.com/thumbnails/Testimonial/v1/${videoNameNoExt}.webp`)
  }
  ...
  const handleThumbnailUpload = async () => {
    ...
    try {
      ...
      const asset_s3_key = 'thumbnails/Testimonial/v1'
  ...
  return (
    <div>
      ...
      {/* Toast Notification */}
      <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] transition-all duration-300 transform ${limitError ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 pointer-events-none'
        }`}>
          ...
          <div className="relative z-10 w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              ...
              <div>
                ...
                {/* Hidden probe image to detect if thumbnail exists */}
                ...
                {/* File picker — shown when no thumbnail exists, or user clicked Replace */}
                {(thumbnailExists === false || showThumbnailPicker) && (
                  <>
                    <label className={`flex items-center gap-2 w-full px-3.5 py-2.5 border rounded-xl cursor-pointer transition-colors bg-stone-50 ${thumbnailError ? 'border-red-300 hover:border-red-400' : 'border-stone-200 hover:border-stone-400'
                      }`}>
                      <svg className="w-3.5 h-3.5 text-stone-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
