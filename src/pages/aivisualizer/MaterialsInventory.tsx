...
function highlight(text: string, query: string) {
    ...
    return (
        <>
            {text.slice(0, idx)}
            <mark className="bg-secondary/25 text-secondary px-[1px]">{text.slice(idx, idx + query.length)}</mark>
        ...
        <div className="flex flex-col h-full animate-in fade-in">
            ...
            <div className="shrink-0 bg-stone-50 border-b border-stone-100 px-4 sm:px-6 pt-4 sm:pt-5 pb-4 sm:pb-5 space-y-3.5 sm:space-y-4">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                    <h2 className="text-sm sm:text-base font-bold color-secondary-dark uppercase tracking-widest flex-1 px-1">Kaira Inventory</h2>
                    ...
                    <button
                        onClick={onBack}
                        className="ml-2 p-1 text-stone-400 hover:color-secondary-dark transition-colors"
                        aria-label="Close"
                    >
                ...
                <div
                    className={`flex items-center gap-2.5 border transition-all duration-200 px-4 py-3 ${isSearchFocused
                ...
                >
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-sm sm:text-sm focus:outline-none placeholder-stone-400 color-secondary-dark min-w-0"
                    />
                    {isSearchMode && (
                        <button
                ...
                {isSearchMode ? (
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-sm bg-secondary/10 text-secondary px-3 py-1 font-bold">"{search.trim()}"</span>
                        {activeFilterCount > 0 && (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative flex shrink-0">
                            <span className="absolute -top-2 left-2 z-10 bg-color-secondary-dark px-1 text-[9px] font-semibold text-stone-300 uppercase tracking-wider leading-none">Type</span>
                            <select
                                value={activeMaterialType}
                                onChange={(e) => setActiveMaterialType(e.target.value)}
                                className="bg-white border border-stone-200 text-xs sm:text-xs px-2.5 py-1.5 h-8 sm:h-9 focus:outline-none focus:border-secondary/60 color-secondary-dark font-bold uppercase tracking-wider cursor-pointer max-w-[100px] sm:max-w-none"
                            >
                        ...
                        <div className="relative shrink-0 w-[120px] sm:w-[145px]" data-col-dropdown>
                            <span className="absolute -top-2 left-2 z-10 bg-color-secondary-dark px-1 text-[9px] font-semibold text-stone-300 uppercase tracking-wider leading-none">Collection</span>
                            <button
                                onClick={() => setShowColDropdown(!showColDropdown)}
                                className="w-full bg-white border border-stone-200 text-xs sm:text-xs px-3 py-1.5 h-8 sm:h-9 flex items-center justify-between hover:border-stone-300"
                            >
                                <span className="font-bold color-secondary-dark uppercase tracking-wider truncate">
                            ...
                            {showColDropdown && (
                                <div className="absolute z-20 top-full left-0 mt-1 bg-white border border-stone-200 shadow-xl p-3 w-[260px] sm:w-[320px] max-h-72 overflow-y-auto">
                                    <div className="grid grid-cols-3 gap-3">
                                        <div
                                            className="flex flex-col items-center gap-1.5 cursor-pointer group"
                                            onClick={() => { setActiveCollection('All'); setShowColDropdown(false) }}
                                        >
                                            <div className={`w-full aspect-square border flex items-center justify-center transition-all ${activeCollection === 'All' ? 'border-secondary shadow-sm bg-secondary/5' : 'border-stone-200 bg-stone-50 group-hover:border-stone-300'}`}>
                                                <span className="text-xs font-bold text-stone-500">ALL</span>
                                            </div>
                                            <span className="text-[9px] sm:text-[10px] font-semibold color-secondary-dark uppercase tracking-wider text-center w-full truncate">All</span>
                                        </div>
                        ...
                        <div className="flex-1 flex items-center min-w-0 gap-1 overflow-hidden relative group/scroll">
                            <button
                                ...
                                className="shrink-0 w-5 h-5 flex items-center justify-center bg-white border border-stone-200 shadow-sm text-stone-400 hover:text-stone-600 z-10"
                            >
                                ...
                                {allColorGroups.filter(c => c !== 'All').map((c) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            title={c}
                                            className={`relative shrink-0 flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 transition-all ${isActive ? 'ring-2 ring-offset-1 ring-secondary scale-110' : 'hover:scale-110 hover:ring-1 hover:ring-stone-300 ring-offset-1 z-10'
                                                }`}
                                        >
                                            <span
                                                className="absolute inset-0 border border-stone-200 shadow-sm"
                ...
                {activeFilterCount > 0 && (
                    <div className="flex flex-wrap gap-2">
                        ...
                        {activeCollection !== 'All' && (
                            <span className="inline-flex items-center gap-2.5 bg-color-secondary-dark text-white text-xs font-bold uppercase tracking-wider pl-2 pr-2.5 py-2 shadow-sm">
                                {(() => {
                                ...
                                <button
                                    ...
                                    className="ml-1 w-4.5 h-4.5 flex items-center justify-center bg-stone-700 hover:bg-stone-500 transition-colors shrink-0"
                        ...
                        {activeColorGroup !== 'All' && (
                            <span className="inline-flex items-center gap-1.5 bg-color-secondary-dark text-white text-xs font-bold uppercase tracking-wider pl-2.5 pr-2 py-1.5 shadow-sm">
                                <span
                    ...
                    <>
                            ...
                            {visibleMaterials.map((m) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`overflow-hidden border-2 relative transition-all ${isSearchMode ? 'aspect-[4/5]' : 'aspect-square'
                                    ...
                                    >
                                        ...
                                        {isActive && (
                                            <div className="absolute top-1 right-1 w-3.5 h-3.5 bg-secondary flex items-center justify-center">
                        ...
                        {hasMore && (
                            <button
                                ...
                                className="mt-3 w-full py-2 border border-stone-200 bg-white text-[12px] sm:text-[11px] font-semibold uppercase tracking-widest text-stone-500 hover:border-secondary/40 hover:text-secondary transition-colors"
                            >
